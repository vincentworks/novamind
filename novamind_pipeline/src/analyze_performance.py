...
def simulate_metrics(persona: str):
    benchmarks = {
...
def aggregate_results_by_persona(audience_results):
    ...
    for row in audience_results:
        ...
        grouped[persona]["open_rate"].append(row["open_rate"])
        grouped[persona]["click_rate"].append(row["click_rate"])
        grouped[persona]["unsubscribe_rate"].append(row["unsubscribe_rate"])
...
def generate_summary(results):
    ...
    return (
        f"{best['persona']} outperformed other segments with a "
        f"{best['click_rate']:.0%} click-through rate, suggesting the messaging was well aligned "
        f"with that audience’s needs. In contrast, {worst['persona']} had the lowest click-through "
...
def main():
    ...
    for person in audience:
        persona = person.get("persona", "Unknown")
        metrics = simulate_metrics(persona)
